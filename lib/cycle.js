...
module.exports = function () {
  ...
  return {
    ...
    registerSuccess: function (req, res, user) {
      ...
      var attempt = {
        attempt_uid: (user.ID || user.id),
        successful: true
      };

      _.merge(attempt, address);

      waterlock.Attempt.create(attempt).catch(function (err) {
        if (err) {
      ...
      if (postResponse === 'jwt') {
        ...
        Jwt.create({ token: jwtData.token, jwt_uid: (user.ID || user.id),revoked:false }).then(function () {
