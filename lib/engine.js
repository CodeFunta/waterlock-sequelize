...
module.exports = function () {
	...
	return {
		 ...
		 * @api private
		 */
		_attachAuthToUser: function (auth, mapUserCriteria, userdefaults, cb) {
			var self = this;
			...
			if (!auth.auth_uid) {
				...
				waterlock.Auth.findOne({
					...
					include: [{ association: "user", include: [ {association: "auth"} ] }]
				})
					.then(function (found) {
						...
						if (found && found.id !== auth.id) {
							...
							waterlock.User.findOrCreate({where: mapUserCriteria, defaults: userdefaults,include: [ {association: "auth"} ]})
				...
				waterlock.User.findById(auth.auth_uid,{include: [ {association: "auth"}] })
					.then(function (user) {
		 ...
		 * @api public
		 */
		findOrCreateAuth: function (criteria, attributes, cb, mapUserCriteria,userdefaults) {
			var self = this;
			waterlock.Auth.findOrCreate({ where: criteria, defaults: attributes, include: [ {association: "user"}]})
				.spread(function (newAuth,created) {
					newAuth.$$created = created;
					self._attachAuthToUser(newAuth, mapUserCriteria, userdefaults, cb);
				}).catch(function (err) {
					waterlock.logger.debug(err);
					return cb(err);
				});
		...
		attachAuthToUser: function (attributes, user, cb) {
			...
			waterlock.User.findOne({where:{id:(user.ID || user.id)}, include:[{association: "auth"}]})
				.then(function (user) {
