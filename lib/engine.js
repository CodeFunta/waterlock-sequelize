	...
	return {
		...
		_attachAuthToUser: function (auth, cb) {
			...
			if (!auth.auth_uid) {
				...
				waterlock.User.findById((auth.user.ID || auth.user.id),{include: [ waterlock.Auth] })
					.then(function (user) {
		...
		findOrCreateAuth: function (criteria, attributes, cb) {
			var self = this;
			waterlock.Auth.findCreateFind({ where: { criteria }, defaults: attributes ,include: [waterlock.User] })
		...
		attachAuthToUser: function (attributes, user, cb) {
			...
			waterlock.User.findOne({where:{id:(user.ID || user.id)}, include:[waterlock.Auth]})
				.then(function (user) {
