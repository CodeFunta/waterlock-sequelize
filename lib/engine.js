...
module.exports = function () {
	...
	return {
		 ...
		 * @api private
		 */
		_attachAuthToUser: function (auth, mapUserCriteria, userdefaults, cb,t) {
			var self = this;
			...
			if (!auth.auth_uid) {
				...
				waterlock.User.findById(auth.auth_uid,{include: [ {association: "auths"}] })
					.then(function (user) {
						...
						if (!_.some(auth.user.auths, {
							...
							return user.setauths(existingAuths).then(()=>{
								cb(null, self._invertAuth(auth));
							});
		...
		findOrCreateAuth: function (criteria, attributes, cb, mapUserCriteria,userdefaults) {
			var self = this;
			return  waterlock.Auth.sequelize.transaction( (t) => {
				waterlock.Auth.findOrCreate({ where: criteria, defaults: attributes, include: [ {association: "user"}] , transaction:t})
				.spread(function (newAuth,created) {
					newAuth.$$created = created;
					self._attachAuthToUser(newAuth, mapUserCriteria, userdefaults, cb,t);
				}).catch(function (err) {
					waterlock.logger.debug(err);
					return cb(err);
				});
		...
		attachAuthToUser: function (attributes, user, cb) {
			...
			waterlock.User.findOne({where:{id:(user.ID || user.id)}, include:[{association: "auths"}]})
				.then(function (user) {
