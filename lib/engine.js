	...
	return {
		...
		_attachAuthToUser: function (auth, cb) {
			...
			if (!auth.userId) {
				...
				waterlock.User.findById(auth.user.id,{include: [ {model:waterlock.Auth} ] })
					.then(function (user) {
		...
		findOrCreateAuth: function (criteria, attributes, cb) {
			var self = this;
			waterlock.Auth.findOrCreate({ where: { criteria }, defaults: attributes ,include: {model:waterlock.User}})
		...
		attachAuthToUser: function (attributes, user, cb) {
			...
			waterlock.User.findOne({where:{id:user.id}, include:[{model: waterlock.Auth}]})
				.then(function (user) {
