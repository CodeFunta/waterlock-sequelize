...
module.exports = function () {
	...
	return {
		 ...
		 * @api private
		 */
		_attachAuthToUser: async (auth, mapUserCriteria, userdefaults) => {
			var self = this;
			...
			if (!auth.auth_uid) {
				...
				let fount = waterlock.Auth.findOne({
		...
		findOrCreateAuth: async (criteria, attributes, mapUserCriteria,userdefaults)  => {
			...
			return  await waterlock.Auth.sequelize.transaction( async (t) => {
				
				let [newAuth,created] = aterlock.Auth.findOrCreate({ where: criteria, defaults: attributes, include: [ {association: "user"}]});
				newAuth.$$created = created;
				return await self._attachAuthToUser(newAuth, mapUserCriteria, userdefaults);
            });
		 ...
		 * @api public
		 */
		attachAuthToUser: async (attributes, user) => {
			var self = this;
			attributes.user = (user.ID || user.id);

			let user = await waterlock.User.findOne({where:{id:(user.ID || user.id)}, include:[{association: "auths"}]});
