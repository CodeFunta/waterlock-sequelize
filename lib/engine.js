...
module.exports = function () {
	...
	return {
		 ...
		 * @api private
		 */
		_attachAuthToUser: function (auth, mapUserCriteria, cb) {
			var self = this;
				...
				waterlock.Auth.findOne({
					...
					.then(function (found) {
						...
						if (found && found.id !== auth.id) {
							...
							waterlock.User.findOrCreate(mapUserCriteria, {
								uid: uuid.v4()
		 ...
		 * @api public
		 */
		findOrCreateAuth: function (criteria, attributes, cb, mapUserCriteria) {
			var self = this;
			waterlock.Auth.findOrCreate({ where: criteria, defaults: attributes, include: [ {association: "user"}]})
				.spread(function (newAuth,created) {
					...
					self._attachAuthToUser(newAuth, mapUserCriteria, cb);
				}).catch(function (err) {
					waterlock.logger.debug(err);
					return cb(err);
				});
